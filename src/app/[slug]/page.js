...
import { watchPagePresence, trackPageVisit } from '@/lib/firebase';
...
function PageVisitorBadge({ slug }) {
  ...
  useEffect(() => {
    ...
    try {
      unsub = watchPagePresence(slug, (pageCount) => {
        setCount(pageCount);
      });
    } catch (error) {
  ...
  return (
    <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
      ...
      <div className="mb-10 flex justify-center">
        <AdSlot slot="3333333333" size="banner" />
      </div>
      ...
      <div className="flex flex-col gap-5">
        {verses.map((verse, i) => (
          <div key={verse.id}>
            ...
            {(i + 1) % 10 === 0 && i < verses.length - 1 && (
              <div className="my-6 flex justify-center">
                <AdSlot slot="4444444444" size="square" />
              </div>
            )}
          </div>
        ))}
      ...
      <div className="mt-12 flex justify-center">
        <AdSlot slot="5555555555" size="banner" />
      </div>
        ...
        <div className="flex items-center justify-center gap-3">
          <button
            onClick={() => {
              ...
              if (navigator.share) {
                navigator.share({ title: meta.title.en, url });
              } else {
    ...
    <LangProvider>
      <div className="min-h-screen bg-[#0d0202]">
        ...
        <div className="fixed inset-0 z-0 pointer-events-none flex items-center justify-center opacity-[0.03]">
          <div
            className="w-[600px] h-[600px] border border-[#c9922a] rounded-full"
            style={{ animation: 'spin 120s linear infinite' }}
          />
          <div
            className="absolute w-[400px] h-[400px] border border-[#c9922a] rounded-full"
