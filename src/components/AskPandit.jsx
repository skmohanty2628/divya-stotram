...
export default function AskPandit({ stotramContext }) {
  ...
  const sendMessage = async () => {
    ...
    const historyToSend = [...messages, { role: 'user', text: userMsg }];
    ...
    try {
      const res = await fetch('/api/ask-pandit', {
        ...
        body: JSON.stringify({
          ...
          history: historyToSend.slice(-6).map(m => ({
      ...
      setMsg(prev => [
        ...prev,
        { role: 'assistant', text: data.reply || 'Please try again. 🙏' }
      ]);
    } catch {
      setMsg(prev => [
        ...prev,
        { role: 'assistant', text: 'The Pandit is meditating. Please try again. 🙏' }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 sm:inset-auto sm:bottom-4 sm:right-4 z-50 sm:w-96 bg-[#0d0202] border border-[#9b59b6]/40 sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col"
              ...
              <div className="grid grid-cols-2 gap-2">
                {CHAT_LANGS.map(l => (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-cinzel-reg text-[10px] tracking-wider text-[#f0c040] group-hover:text-[#f5c842]">
                        {l.name}
                      </p>
                      <p className="font-garamond text-[11px] text-[#e8d5b5]/50">
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                ...
                {messages.map((m, i) => (
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div
                      className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm font-garamond leading-relaxed ${
                        m.role === 'user'
                    ...
                    <div className="bg-[#1a0505] border border-[#c9922a]/20 rounded-2xl rounded-bl-sm px-4 py-2.5">
                      <div className="flex gap-1">
                        {[0, 150, 300].map(d => (
                          <span
                            key={d}
                            className="w-1.5 h-1.5 rounded-full bg-[#c9922a] animate-bounce"
              ...
              <div className="px-3 pb-3 pt-2 border-t border-[#c9922a]/10">
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder={
                      chatLang.code === 'hi' ? 'अपना प्रश्न पूछें...' :
