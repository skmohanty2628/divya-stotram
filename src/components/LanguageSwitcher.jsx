...
export function LangProvider({ children }) {
  ...
  useEffect(() => {
    const urlLang = searchParams.get('lang');
    const newLang = ['en', 'hi', 'od', 'te'].includes(urlLang) ? urlLang : 'en';
    if (newLang !== lang) {
  ...
  const setLang = (newLang) => {
    ...
    if (newLang === 'en') {
      params.delete('lang'); // Clean URL for English (default)
    } else {
...
export default function LanguageSwitcher() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="font-cinzel-reg text-xs tracking-wider text-[#8b1a00] font-bold">
          Language
        </span>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 bg-white border border-[#c9922a]/30 rounded-xl overflow-hidden z-50 min-w-[150px] shadow-xl shadow-black/10">
          <p className="font-cinzel-reg text-[9px] tracking-[2px] uppercase text-[#c9922a]/50 px-3 pt-3 pb-1">
            Select Language
          </p>
          {LANGS.map((l) => (
            <button
            ...
            >
              ...
              {l.code === lang && (
                <span className="ml-auto text-[#c9922a] text-xs">✓</span>
              )}
            </button>
          ))}
        </div>
